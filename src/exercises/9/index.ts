...
type ServerTimeResponse = ApiResponse<number>;
type CoffeeMachineResponse = ApiResponse<number>;
...
export function requestCurrentServerTime(callback: (response: ServerTimeResponse) => void) {
    callback({
        status: 'success',
        data: Date.now()
    });
...
export function requestCoffeeMachineQueueLength(callback: (response: CoffeeMachineResponse) => void) {
    callback({
        status: 'error',
